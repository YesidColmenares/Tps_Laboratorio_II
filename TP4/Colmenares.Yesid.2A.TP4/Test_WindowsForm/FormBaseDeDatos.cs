...
namespace Test_WindowsForm
{
    public partial class FormBaseDeDatos : Form
    {
            ...
            if (editar.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    if (this.Sql.Agregar(editar.Producto))
                    {
                        ...
                        this.LlenarListBox(this.lstBoxMostrar, this.Sql.ObtenerLista(this.txtNombreTabla.Text));
                    }
                }
        ...
        {
            List<Producto> lista = this.Sql.ObtenerLista(this.txtNombreTabla.Text);

            if (lista is not null)
            {
                this.ActivarYEstablecerTitulo(this.lstBoxMostrar, "LISTA DE PRODUCTOS");
                this.txtNombreTabla.Enabled = false;
        ...
        public Sql Sql
        {
            get
            {
                return new Sql(this.txtNombreServidor.Text, this.txtNombreBaseDatos.Text, this.txtNombreTabla.Text);
            }
        }
    }
}
